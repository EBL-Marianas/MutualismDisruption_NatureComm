...
jags.PS.pexp.all <- jags.parallel(data = c("site.island","trap.site","dist", "cc",
...
print(jags.PS.pexp.all,2);plot(jags.PS.pexp.all)

# Plot chains and histograms
plot(mcmc.list(mapply(function(d,x) mcmc(x[,d,!is.element(substring(dimnames(x)[[3]],1,3), c("fit","ll."))]), 1:3, list(x=jags.PS.pexp.all$BUGS$sims.array), SIMPLIFY=F)), ask=T)
...
# Call JAGS to fit the model
jags.PS.pexp.G <- jags.parallel(data = c("site.island","trap.site","dist", "cc",
                                         "trap.area","basal.area","kern.indx","seeds"), 
                                inits = inits.pexp.G, 
                                parameters.to.save = c("ll.data","dev.data","dev.lfec","mu.lfec","sigma.lfec","lfec","lscale.grp","lshape.grp","ldmean.grp","ldmode.grp","r1","r2","fit"), 
                                model.file = "HBdisp.pexp.r", 
...
# Plot and print summaries of results
print(jags.PS.pexp.G,2);plot(jags.PS.pexp.G)

# Plot chains and histograms
plot(mcmc.list(mapply(function(d,x) mcmc(x[,d,!is.element(substring(dimnames(x)[[3]],1,3), c("fit","ll."))]), 1:3, list(x=jags.PS.pexp.G$BUGS$sims.array), SIMPLIFY=F)), ask=T)
...
# Call JAGS to fit the model
jags.PS.pexp.GRST <- jags.parallel(data = c("site.island","trap.site","dist", "cc",
                                            "trap.area","basal.area","kern.indx","seeds"), 
...
# Plot and print summaries of results
print(jags.PS.pexp.GRST,2);plot(jags.PS.pexp.GRST)

# Plot chains and histograms
plot(mcmc.list(mapply(function(d,x) mcmc(x[,d,!is.element(substring(dimnames(x)[[3]],1,3), c("fit","ll."))]), 1:3, list(x=jags.PS.pexp.GRST$BUGS$sims.array), SIMPLIFY=F)), ask=T)
...
# Call JAGS to fit the model
jags.PS.2Dt.all <- jags.parallel(data = c("site.island","trap.site","dist", "cc",
                                          "trap.area","basal.area","kern.indx","seeds"), 
                                 inits = inits.2Dt.all, 
                                 parameters.to.save = c("ll.data","dev.data","dev.lfec","mu.lfec","sigma.lfec","lfec","lscale.grp","lshape.grp","ldmean.grp","ldmode.grp","r1","r2","fit"), 
...
# Plot and print summaries of results
print(jags.PS.2Dt.all,2);plot(jags.PS.2Dt.all)

# Plot chains and histograms
plot(mcmc.list(mapply(function(d,x) mcmc(x[,d,!is.element(substring(dimnames(x)[[3]],1,3), c("fit","ll."))]), 1:3, list(x=jags.PS.2Dt.all$BUGS$sims.array), SIMPLIFY=F)), ask=T)
...
# Call JAGS to fit the model
jags.PS.2Dt.G <- jags.parallel(data = c("site.island","trap.site","dist", "cc",
                                        "trap.area","basal.area","kern.indx","seeds"), 
                               inits = inits.2Dt.G, 
                               parameters.to.save = c("ll.data","dev.data","dev.lfec","mu.lfec","sigma.lfec","lfec","lscale.grp","lshape.grp","ldmean.grp","ldmode.grp","r1","r2","fit"), 
                               model.file = "HBdisp.2Dt.r", 
...
# Plot and print summaries of results
print(jags.PS.2Dt.G,2);plot(jags.PS.2Dt.G)

# Plot chains and histograms
plot(mcmc.list(mapply(function(d,x) mcmc(x[,d,!is.element(substring(dimnames(x)[[3]],1,3), c("fit","ll."))]), 1:3, list(x=jags.PS.2Dt.G$BUGS$sims.array), SIMPLIFY=F)), ask=T)
...
# Call JAGS to fit the model
jags.PS.2Dt.GRST <- jags.parallel(data = c("site.island","trap.site","dist", "cc",
                                           "trap.area","basal.area","kern.indx","seeds"), 
...
# Plot and print summaries of results
print(jags.PS.2Dt.GRST,2);plot(jags.PS.2Dt.GRST)

# Plot chains and histograms
plot(mcmc.list(mapply(function(d,x) mcmc(x[,d,!is.element(substring(dimnames(x)[[3]],1,3), c("fit","ll."))]), 1:3, list(x=jags.PS.2Dt.GRST$BUGS$sims.array), SIMPLIFY=F)), ask=T)
...
mod.list <- list(jags.PS.pexp.all$BUGS, jags.PS.pexp.G$BUGS, jags.PS.pexp.GRST$BUGS,
                 jags.PS.2Dt.all$BUGS, jags.PS.2Dt.G$BUGS, jags.PS.2Dt.GRST$BUGS)
